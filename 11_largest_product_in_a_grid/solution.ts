...
function* creatCols(rows: number[][]): Generator<number[]> {
...
const creatDiagon1 = (rows: number[][]): number[][] => {
  const diagons: number[][] = [];
...
const creatDiagons2 = (rows: number[][]): number[][] => {
  const diagons: number[][] = [];
...
const consecutiveProduct = (line: number[]): number => {
  let i = 0;
...
const largestProducts = (series: number[][], n = 4): number => {
  return series
    .map((line) => consecutiveProduct(line))
    .reduce((max, ele) => Math.max(max, ele));
};

const largestProduct = (grid: string): number => {
  const rows = grid
    .split("\n")
